...
import static com.example.art.shouye.artwork.ArtworkStaticData.ARTWORK_COVER;
import static com.example.art.shouye.artwork.ArtworkStaticData.ARTWORK_ID;
import static com.example.art.shouye.artwork.ArtworkStaticData.ARTWORK_NAME;
import static com.example.art.shouye.artwork.ArtworkStaticData.ARTWORK_URL;
import static com.example.art.shouye.artwork.ArtworkStaticData.IS_COLLECTED;

public class UrlForArtworkActivity extends AppCompatActivity {
    private WebView myWebView;
    private long exitTime = 0;
    private LikeButton collectArtworkLikeButton;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_url_for_artwork);
        //添加用户历史部分
        Date date = new Date(System.currentTimeMillis());
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        addHistory(ARTWORK_NAME,simpleDateFormat.format(date),USERID);
        ...
        //收藏艺术品按钮
        collectArtworkLikeButton = findViewById(R.id.collectArtworkLikeButton);
        collectArtworkLikeButton.setLiked(IS_COLLECTED);
        collectArtworkLikeButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if(!collectArtworkLikeButton.isLiked()){
                    addCollectionForArtwork();
                    collectArtworkLikeButton.setLiked(true);
                }else{
                    deleteCollectionForArtwork();
                    collectArtworkLikeButton.setLiked(false);
                }
            }
        });
    }
    private void addCollectionForArtwork(){
        ...
        try {
            ...
            data.put("collectionId",ARTWORK_ID);
            //这里得类型非常重要，1表示是艺术品类型，还有2和3
            data.put("type",1);
            data.put("cover",ARTWORK_COVER);
            data.put("name",ARTWORK_NAME);
            data.put("url",ARTWORK_URL);
            body.put("data",data);
