...
public class UrlForSearchActivity extends AppCompatActivity {
    private WebView myWebView;
    private long exitTime = 0;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_url_for_search);
        Intent intent = getIntent();
        ArtSuggestion artSuggestion = getIntent().getExtras().getParcelable("data");

        myWebView = (WebView) findViewById(R.id.webView);
        myWebView.getSettings().setJavaScriptEnabled(true);  //设置WebView属性,运行执行js脚本
        myWebView.loadUrl(artSuggestion.getMyUrl());    //设置网址
