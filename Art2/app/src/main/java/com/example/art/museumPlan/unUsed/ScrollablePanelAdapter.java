...
public class ScrollablePanelAdapter extends PanelAdapter {
    ...
    private List<MuseumInfo> museumInfoList=new ArrayList<>();
        ...
        switch (viewType) {
            ...
            case ROOM_TYPE:
                return new RoomViewHolder(LayoutInflater.from(parent.getContext())
                        .inflate(R.layout.listitem_museum_info, parent, false));
            case ORDER_TYPE:
    ...
    private void setRoomView(int pos, RoomViewHolder viewHolder) {
        MuseumInfo museumInfo = museumInfoList.get(pos - 1);
        if (museumInfo != null && pos > 0) {
            viewHolder.museumLocationTextView.setText(museumInfo.getMuseumLocation());
            viewHolder.museumNameTextView.setText(museumInfo.getMuseumName());
        }
    }
    ...
    private void setOrderView(final int row, final int column, OrderViewHolder viewHolder) {
        ...
        if (orderInfo != null) {
            if (orderInfo.getStatus() == OrderInfo.Status.BLANK) {
            ...
            } else if (orderInfo.getStatus() == OrderInfo.Status.UNFINISHED) {
                ...
                viewHolder.view.setBackgroundResource(orderInfo.isBegin() ? R.drawable.bg_museum_red_begin_with_stroke : R.drawable.bg_museum_red_with_stroke);
            } else if (orderInfo.getStatus() == OrderInfo.Status.SET_OUT) {
                viewHolder.nameTextView.setText(orderInfo.isBegin() ? orderInfo.getGuestName() : "");
                viewHolder.statusTextView.setText(orderInfo.isBegin() ? "reverse" : "");
                viewHolder.view.setBackgroundResource(orderInfo.isBegin() ? R.drawable.bg_museum_blue_begin_with_stroke : R.mipmap.bg_museum_blue_middle);
            }
            if (orderInfo.getStatus() != OrderInfo.Status.BLANK) {
    ...
    private static class RoomViewHolder extends RecyclerView.ViewHolder {
        public TextView museumLocationTextView;
        public TextView museumNameTextView;

        public RoomViewHolder(View view) {
            super(view);
            this.museumLocationTextView = (TextView) view.findViewById(R.id.museum_location);
            this.museumNameTextView = (TextView) view.findViewById(R.id.museum_name);
        }
    }
    ...
    public void setRoomInfoList(List<MuseumInfo> museumInfoList) {
        this.museumInfoList = museumInfoList;
    }
